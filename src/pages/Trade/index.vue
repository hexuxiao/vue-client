  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="address in trandeInfo.userAddressList"
        :key="address.id"
        @click="selectedAddress=address"
      >
        <template @click="selectedAddress=address">
          <span class="username" :class="{selected:address===selectedAddress}">{{address.consignee}}</span>
          <p>
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="item in trandeInfo.detailArrayList" :key="item.skuId">
          <li>
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b>
            <i>{{trandeInfo.totalNum}}</i>件商品，总商品金额
          </b>
          <span>¥{{trandeInfo.totalAmount}}</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
...
export default {
  ...
  data() {
    return {
      selectedAddress: {}, //选中的收获地址
      orderComment: "" //订单备注
    };
  },
  watch: {
    "trandeInfo.userAddressList"(value) {
      //找出默认地址
  ...
  computed: {
    trandeInfo() {
      return this.$store.state.order.trandeInfo;
    }
  },

  mounted() {
    this.$store.dispatch("getTrandeInfo");
  },
  methods: {
    //发送提交订单的请求
    async submitOrder() {
